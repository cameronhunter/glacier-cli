...
public class Download implements Callable<File> {
    ...
    public Download(ArchiveTransferManager transferManager, String vault, String archiveId) {
        this(transferManager, vault, archiveId, DEFAULT_PROGRESS_LISTENER);
    }

    public Download(ArchiveTransferManager transferManager, String vault, String archiveId, ProgressListener progressListener) {
        this.transferManager = notNull(transferManager);
        this.vault = notBlank(vault);
        this.archiveId = notBlank(archiveId);
        this.progressListener = notNull(progressListener);
    }
    ...
    File downloadTo(File file) {
        try {
            ...
            transferManager.download(null, vault, archiveId, file, progressListener);
